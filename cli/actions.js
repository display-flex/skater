...
function checkConfig(options) {
    if (!options.parent.server && !config.server()) {
        console.log('Run %s first, or provide %s', 'skater login'.bold, '--server <url>'.bold);
        process.exit(1);
    }
    ...
    if (!options.parent.token && !config.accessToken()) {
        console.log('Run %s first or provide %s', 'skater login'.bold, '--token <access token>'.bold);
        process.exit(1);
    }
    ...
    request.get(gServer + API + filePath, { qs: gQuery }, function (error, result, body) {
        ...
        if (result.statusCode === 222) {
            ...
            if (!files || files.entries.length === 0) {
                console.log('No files on the server. Use %s to upload some.', 'skater put <file>'.yellow);
            } else {
                console.log('Entries:');
